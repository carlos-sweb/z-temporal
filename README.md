...
This is a large API, built in phases. **Phase 1** covers `PlainDate`/`PlainTime`/`PlainDateTime` construction, parsing/formatting, and field access — the ISO calendar only, no arithmetic yet. **Phase 2** adds `Duration`. **Phase 3a** adds `.add()`/`.subtract()`. **Phase 3b** adds `.until()`/`.since()`. **Phase 3c (current)** adds `.round()` on the Plain types and `Duration.round()`/`.total()`, closing out "Phase 3: arithmetic" entirely.
